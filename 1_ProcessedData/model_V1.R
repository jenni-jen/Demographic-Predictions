...
train = read.csv("train.csv",stringsAsFactors = F, fileEncoding='utf-8')
validation = read.csv("validation.csv",stringsAsFactors = F, fileEncoding='utf-8')
...
get_accuracy_logi(logistic_fit1, valid_4_logistic) # accuracy = 0.7109111
...
sum(p_forest1==validation$gender)/dim(validation)[1] # accuracy = 0.7126824
...
which.max(tree_accuracy) # 第9次accuracy最高，即ntree=400, mtry=2
tree_accuracy[9] # accuracy = 0.7171717

set.seed(123)
my_forest_best = randomForest(as.factor(gender)~events_holi_day+events_holi_night+events_work_day+
                                events_work_night+education+entertainment+finance+game+health+
                                life+news+shopping+social+tools+travel,
                              data=train,ntree=400,mtry=2,importance=T)
...
sum(pred_xgb==validation$gender)/length(validation$gender) # acurracy = 0.684624
...
sum(nn_pred==validation$gender)/length(validation$gender) # accuracy = 0.7160494
...
sum(nn_pred==validation$gender)/length(validation$gender) # accuracy = 0.7003367
